...
ScavTrap::ScavTrap()    {
    ...
    std::cout << "ScavTrap default contructor called for <" << getName() << ">.\n\n";
}

ScavTrap::ScavTrap(std::string name) {
    this->_name = name;
    initAttribs(100, 50, 20);
    std::cout << "ScavTrap named constructor called for <" << getName() << ">.\n\n";
}
...
void ScavTrap::attack(std::string const & target){
    ...
    << this->_energyPoints << ").\n";
    std::cout << "ScavTrap <" << _name << "> got (1) [hit point] per attack and now [his points] = (" << _hitpoints << ").\n\n";
}

ScavTrap::~ScavTrap()   {
    std::cout << "ScavTrap destructor called and destroyed <" << getName() << ">.\n";
}
